...
const LoginPage = () => {
  ...
  const checkForFormRemember = () => {
    ...
    if (rememberMe) {
      ...
      localStorage.setItem('loginCred', JSON.stringify(dataToStore))
    }
    else {
  ...
  useEffect(() => {
    if (settings.user.isLoggedIn) router.replace('/')
  ...
  useEffect(() => {
    ...
    if (loginCred) {
      const { username, password } = JSON.parse(localStorage.getItem('loginCred'))
        ...
        <Card sx={{ zIndex: 1 }}>
          <CardContent sx={{ padding: theme => `${theme.spacing(12, 9, 7)} !important` }}>
            ...
            <form noValidate autoComplete='off' onSubmit={e => e.preventDefault()}>
              ...
              >
                <FormControlLabel control={<Checkbox />} label='Remember Me' onChange={handleChange('rememberMe')} checked={values.rememberMe} />
                <Link passHref href='/'>
